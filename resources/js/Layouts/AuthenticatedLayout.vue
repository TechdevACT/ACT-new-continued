...
const navClasses = (active) =>
  active
    ? 'inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg shadow-sm'
    : 'inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors';
</script>
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
    ...
    <div class="fixed inset-0 -z-10 overflow-hidden">
      <div class="absolute -top-40 -right-40 h-80 w-80 rounded-full bg-indigo-200/30 blur-3xl"></div>
        ...
        <div class="flex h-16 items-center justify-between">
          ...
          <div class="hidden lg:block">
            <Dropdown align="right" width="48">
              <template #trigger>
                <button class="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:border-indigo-300 hover:shadow-md transition-all">
                  <img v-if="$page.props.auth.user.avatar"
                    class="h-8 w-8 rounded-lg object-cover"
                    :src="$page.props.auth.user.avatar"
                    :alt="$page.props.auth.user.name">
                  <div v-else class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 text-sm font-bold text-white">
      ...
      <div :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }" class="lg:hidden border-t border-slate-200 bg-white">
        <div class="space-y-1 px-4 py-3">
          <Link :href="route('dashboard')"
            ...
            :class="route().current('dashboard') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'">
          ...
          <Link :href="route('projectsSetting.index')"
            class="flex items-center gap-2 rounded-lg px-3 py-2 text-base font-medium"
            :class="route().current('projectsSetting.index') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
            Projects
          ...
          <Link :href="route('blog.index')"
            class="flex items-center gap-2 rounded-lg px-3 py-2 text-base font-medium"
            :class="route().current('blog.index') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            Blog
          ...
          <Link :href="route('settings.index')"
            class="flex items-center gap-2 rounded-lg px-3 py-2 text-base font-medium"
            :class="(route().current('settings.index') || route().current('settings.edit')) ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
        ...
        <div class="border-t border-slate-200 bg-slate-50 p-4">
          <div class="flex items-center gap-3 mb-3">
            <img v-if="$page.props.auth.user.avatar"
              ...
              :src="$page.props.auth.user.avatar"
              alt="">
            <div v-else class="h-10 w-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold">
              {{ $page.props.auth.user.name.charAt(0) }}
            </div>
